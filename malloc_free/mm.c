...
static void coalesceFreeBlock(BlockInfo* oldBlock) {
  ...
  if (newSize != oldSize) {
    ...
    *(size_t*)UNSCALED_POINTER_SUB(blockCursor, WORD_SIZE) = newSize | TAG_PRECEDING_USED;  
    ...
    if (newBlock->sizeAndTags!=(*((size_t*)UNSCALED_POINTER_ADD(newBlock,SIZE(newBlock->sizeAndTags)-WORD_SIZE)))) {
        printf("critical points21212!!!!!\n");
    }
  return;
}
...
void* mm_malloc (size_t size) {
  ...
  size_t blockSize;
  size_t frontBlockSize;
  BlockInfo* remainBlock;
  void* retPointer;
  ...
  // You will want to replace this return statement...
    ptrFreeBlock=searchFreeList(size);
    ...
    if (blockSize-reqSize>MIN_BLOCK_SIZE) {
     ptrFreeBlock->sizeAndTags=(reqSize|(ptrFreeBlock->sizeAndTags&7));
     remainBlock=(BlockInfo*)UNSCALED_POINTER_ADD(ptrFreeBlock,reqSize);
     remainBlock->sizeAndTags=(blockSize-reqSize)|TAG_PRECEDING_USED;
     ((BlockInfo*)UNSCALED_POINTER_ADD(ptrFreeBlock,blockSize-WORD_SIZE))->sizeAndTags=(blockSize-reqSize)|TAG_PRECEDING_USED;
     insertFreeBlock(remainBlock);
     coalesceFreeBlock(remainBlock);
     }
    else
    {
    //*((size_t*)(UNSCALED_POINTER_ADD(ptrFreeBlock,blockSize-WORD_SIZE)))=ptrFreeBlock->sizeAndTags;
    BlockInfo* frontBlock=UNSCALED_POINTER_ADD(ptrFreeBlock,blockSize);
    frontBlock->sizeAndTags=frontBlock->sizeAndTags|TAG_PRECEDING_USED;
    frontBlockSize=SIZE(frontBlock->sizeAndTags);
    if ((SIZE(frontBlock->sizeAndTags)>0)&&((frontBlock->sizeAndTags&TAG_USED)==0)) {
        ((BlockInfo*)UNSCALED_POINTER_ADD(frontBlock,frontBlockSize-WORD_SIZE))->sizeAndTags=frontBlock->sizeAndTags;
    }
    }
    //printf("the pointer returned: %p\n",retPointer);
    //printf("the heap footer: %p\n",UNSCALED_POINTER_SUB(mem_heap_hi(),WORD_SIZE-1));
    /*if (ptrFreeBlock->sizeAndTags!=*((size_t*)(UNSCALED_POINTER_ADD(ptrFreeBlock,blockSize-WORD_SIZE)))) {
      printf("critical points!!!!!!!!!\n");
      exit(1);
    }*/
    retPointer=UNSCALED_POINTER_ADD(ptrFreeBlock,WORD_SIZE);
    return retPointer;
}
...
void mm_free (void *ptr) {
  ...
  BlockInfo * blockfooter;
  BlockInfo * nextBlock;
  // Implement mm_free.  You can change or remove the declaraions
  // above.  They are included as minor hints.
  blockheader=UNSCALED_POINTER_SUB(ptr,WORD_SIZE);
  blockheader->sizeAndTags=blockheader->sizeAndTags&(0xfffffffe);
  blockSize=SIZE(blockheader->sizeAndTags);
  blockfooter=UNSCALED_POINTER_ADD(ptr,blockSize-2*WORD_SIZE);
  blockfooter->sizeAndTags=blockheader->sizeAndTags;
  nextBlock=(BlockInfo*)UNSCALED_POINTER_ADD(blockheader,SIZE(blockheader->sizeAndTags));
  nextBlock->sizeAndTags=nextBlock->sizeAndTags&(0xfffffffd);
    
    if ((SIZE(nextBlock->sizeAndTags)>0)&&((nextBlock->sizeAndTags&TAG_USED)==0)) {
        *((size_t*)UNSCALED_POINTER_ADD(nextBlock,SIZE(nextBlock->sizeAndTags)-WORD_SIZE))=nextBlock->sizeAndTags;
    }
  insertFreeBlock(blockheader);
    //printf("free block header %zu\n",blockheader->sizeAndTags);
    /*if (blockSize==8192) {
        printf("checkpoint 2823\n");
        printf("next block used tag: %zu, size: %zu\n",nextBlock->sizeAndTags&TAG_USED,SIZE(nextBlock->sizeAndTags));
        printf("pre block size: %zu\n",*((size_t*)UNSCALED_POINTER_SUB(ptr,2*WORD_SIZE)));
    }*/
  coalesceFreeBlock(blockheader);
    //printf("free block header %zu\n",blockheader->sizeAndTags);
  //printf("checkpoint8\n");
  return;
}
...
int mm_check() {
    ...
    size_t currentSize;
    size_t currentBlockFooter;
    currentBlock=(void*)(((char*)mem_heap_lo())+WORD_SIZE);
    while (((char*)((void*)currentBlock))<(((char*)mem_heap_hi())-WORD_SIZE+1)) {
        currentSize=SIZE(currentBlock->sizeAndTags);
        currentBlockFooter=(*((size_t*)(UNSCALED_POINTER_ADD(currentBlock,currentSize-WORD_SIZE))));
        if (currentBlock->sizeAndTags!=currentBlockFooter) {
            printf("currentBlock header size and tags: %zu\n",currentBlock->sizeAndTags);
            printf("currentBlock footer size and tags: %zu\n",currentBlockFooter);
            printf("current block footer address: %p, heap footer address: %p\n",UNSCALED_POINTER_ADD(currentBlock,currentSize-WORD_SIZE),mem_heap_hi()-WORD_SIZE+1);
            return -1;
        }
        currentBlock=(void*)(((char*)currentBlock)+currentSize);
    }
    return 0;
}
...
void mm_status()
{
    ...
    size_t currentSize;
    size_t currentBlockFooter;
    printf("heap low address: %p\n",mem_heap_lo());
    currentBlock=(void*)(((char*)mem_heap_lo())+WORD_SIZE);
    while (((char*)((void*)currentBlock))<(((char*)mem_heap_hi())-7)) {
        currentSize=SIZE(currentBlock->sizeAndTags);
        currentBlockFooter=(*((size_t*)(UNSCALED_POINTER_ADD(currentBlock,currentSize-WORD_SIZE))));
        printf("currentBlock address, size and tags: %p,%zu,%zu\n",currentBlock,SIZE(currentBlock->sizeAndTags),((currentBlock->sizeAndTags)&0x7));
        printf("currentBlock footer, size and tags: %zu\n",currentBlockFooter);
        printf("current block footer address: %p\n",UNSCALED_POINTER_ADD(currentBlock,currentSize-WORD_SIZE));
        currentBlock=(void*)(((char*)currentBlock)+currentSize);
    }
    if (FREE_LIST_HEAD!=NULL) {
        printf("first free block address, size and tags: %p,%zu\n",FREE_LIST_HEAD,(*((BlockInfo*)(FREE_LIST_HEAD))).sizeAndTags);
    }
    else
    {
        printf("first free block address: null\n");
    }
    
    printf("heap footer address: %p,size and tags: %zu\n",UNSCALED_POINTER_SUB(mem_heap_hi,WORD_SIZE+1),(*((size_t*)(UNSCALED_POINTER_SUB(mem_heap_hi,WORD_SIZE+1)))));
    return;
}
